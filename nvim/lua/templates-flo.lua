...
local templates_config = vim.fn.stdpath("data") .. "/templates.json"
...
vim.api.nvim_set_hl(0, "TplNormal", { bg = colors.bg, fg = colors.fg })
vim.api.nvim_set_hl(0, "TplBorder", { fg = colors.border, bg = colors.bg })
vim.api.nvim_set_hl(0, "TplName", { fg = colors.blue, bold = true })
vim.api.nvim_set_hl(0, "TplDesc", { fg = colors.dim_fg, italic = true })
vim.api.nvim_set_hl(0, "TplSeparator", { fg = colors.border })
vim.api.nvim_set_hl(0, "TplStats", { fg = colors.border, bold = true })
vim.api.nvim_set_hl(0, "TplTitle", { fg = colors.blue, bg = colors.bg, bold = true })
vim.api.nvim_set_hl(0, "TplSidebarSel", { fg = colors.blue, bold = true })

local function save_data(data)
    local f = io.open(templates_config, "w")
    if f then f:write(vim.fn.json_encode(data)) f:close() end
end

local function load_data()
    local f = io.open(templates_config, "r")
    if f then
        local content = f:read("*a")
        f:close()
        local ok, decoded = pcall(vim.fn.json_decode, content)
        if ok and decoded.templates then return decoded end
    end
    return { templates = {} }
end

local tpl_data = load_data()

local function open_templates_manager()
    ...
    local ns_id = vim.api.nvim_create_namespace("tpl_tui_selector")
    local sel_ns_id = vim.api.nvim_create_namespace("tpl_tui_overlays")

    local win = vim.api.nvim_open_win(bufnr, true, {
        relative = "editor", width = width, height = height,
        row = row, col = col, style = "minimal", border = "single",
        title = { { "    Plantillas ", "TplTitle" } }, title_pos = "center"
    })
    vim.api.nvim_set_option_value("winhl", "Normal:TplNormal,FloatBorder:TplBorder", { win = win })
    ...
    local function attach_behavior_hooks()
        pcall(vim.api.nvim_clear_autocmds, { buffer = bufnr, group = "TplTUIBehavior" })
        vim.api.nvim_create_autocmd({ "InsertLeave", "WinLeave" }, {
            buffer = bufnr, group = "TplTUIBehavior",
    ...
    vim.api.nvim_create_augroup("TplTUIBehavior", { clear = true })
    ...
    local function render()
        ...
        for i, item in ipairs(tpl_data.templates) do
            if filter == "" or item.name:lower():find(filter, 1, true) or (item.desc or ""):lower():find(filter, 1, true) then
        ...
        vim.api.nvim_buf_add_highlight(bufnr, ns_id, "TplTitle", 0, 0, 1)
        vim.api.nvim_buf_add_highlight(bufnr, ns_id, "TplSeparator", 1, 0, -1)
        ...
        for buf_line_idx, mapping in pairs(line_map) do
            if mapping.is_sel then
                vim.api.nvim_buf_set_extmark(bufnr, sel_ns_id, buf_line_idx - 1, 0, {
                    virt_text = { { "│", "TplSidebarSel" } },
                    virt_text_pos = "overlay"
                })
            end

            if mapping.part == "main" then
                vim.api.nvim_buf_add_highlight(bufnr, ns_id, "TplName", buf_line_idx - 1, 4, -1)
            elseif mapping.part == "sub" then
                vim.api.nvim_buf_add_highlight(bufnr, ns_id, "TplDesc", buf_line_idx - 1, 6, -1)
            end
        end
        ...
        vim.api.nvim_win_set_config(win, { footer = { { stats_str, "TplStats" } }, footer_pos = "right" })
        pcall(vim.api.nvim_win_set_cursor, win, { 1, prefix_len + #current_search_text })
    end

    _G.trigger_templates_render = function()
    ...
    vim.keymap.set("i", "<CR>", function()
        ...
        if vim.api.nvim_win_is_valid(parent_editor_win) then
            ...
            local content = item.data.content or ""
    ...
    vim.keymap.set("i", "<C-a>", function() spawn_input_create("-p ") end, opt_i)
    ...
    vim.keymap.set("i", "<C-d>", function()
        ...
        require("input-remove").confirm_delete({
            ...
            label = "Plantilla",
            on_confirm = function()
                table.remove(tpl_data.templates, item.idx)
                save_data(tpl_data)
                _G.trigger_templates_render()
            end,
            on_cancel = function()
...
vim.keymap.set("n", "<leader>p", open_templates_manager)
